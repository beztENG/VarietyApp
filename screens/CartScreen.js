...
export default function CartScreen() {
    ...
    return (
        <View className="bg-white flex-1" style={{ marginTop: 50, }}>
            {/* Back button */}
            <View className="relative py-4 shadow-sm">
                ...
                <View>
                    ...
                    <Text className="text-center text-gray-500">{shop?.name || 'No shop selected'}</Text>
                </View>
            </View>
            ...
            >
                {cartItems.length === 0 ? (
                    <Text className="text-center mt-20">Your cart is empty</Text>
                ) : (
                    cartItems.map((item) => (
                        <View
                            key={item._id}
